...
app.use(session({secret: "jhgfoweriwoerodfkvxcvmxvxm12340fsdfkl32f0y0reofasf", saveUninitialized: true, resave: false}))
async function AuthMiddleware(req, res, next) {
  if (req.session && req.session.userID) {
    let user = await model.User.findOne({ _id: req.session.userID })
    if (!user) {
      console.log(user, "You didn't find the user.")
...
app.post("/users", async function (req, res) {
  try {
    let newUser = await new model.User({email: req.body.email, name: req.body.name})
...
app.delete("/users/:userID", AuthMiddleware, async function (req, res) {
  try {
    let isDeleted = await model.User.findOneAndDelete({_id: req.params.userID})
    if (!isDeleted) {
      res.status(404).send("User Not Found")
      return
    }
...
app.put("/quizzes/:quizID", AuthMiddleware, async function (req, res) {
  try {
    let quiz = await model.Quiz.findOne({_id: req.params.quizID, owner: req.user._id}).populate("owner")
    if (!quiz) {
      res.status(404).send("Quiz not found")
      return
    }
...
app.delete("/quizzes/:quizID", AuthMiddleware, async function (req, res) {
  try {
    let isDeleted = await model.Quiz.findOneAndDelete({_id: req.params.quizID, owner: req.user._id})
    if (!isDeleted) {
      res.status(404).send("Quiz Not Found")
      return
    }
